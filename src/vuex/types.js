/**
 * Create by zechun.chen on 2016/12/22
 */

//公共
export const COM_LOADING_STATUS = 'COM_LOADING_STATUS'
export const COM_CONF = 'COM_CONF'
export const CHANGE_LEFTNAV_STATUS = 'CHANGE_LEFTNAV_STATUS'

//index页面操作
export const GET_CAROUSEL_DATA = 'GET_CAROUSEL_DATA'
export const GET_ARTICLE_DATA = 'GET_ARTICLE_DATA'


//栏目页面
export const GET_CATE_DATA = 'GET_CATE_DATA'
export const SET_CATE_ARTICLES_STATUS = 'SET_CATE_ARTICLES_STATUS'
export const GET_CATE_ARTICLES_DATA = 'GET_CATE_ARTICLES_DATA'


//内容页面
export const GET_CONTENT_ARTICLE_DATA = 'GET_CONTENT_ARTICLE_DATA'
